...
bool Table::add(const myNode*        node,
                         ...
                         const Value     &value,
                         unsigned int          source,
                         bool                  sadPrecedence, // set_precedence value for checking in whole hierachy
                         int                   level)
{
  if (! done) {
    if (! inValid) {
      ...
      if (instance().getAttr("err")) std::cout("ERR_001", __LINE__);
      return false;
    }
    initialize(level);
  }
  ...
  if (index < tables.size()) {
    if (tables[index]) {
      return tables[index]->set(nodePointer, node, value, value, source, sadPrecedence, level);
    }
  }
  ...
  if (instance().getAttr("err")) {
    ...
    std::cout("NOTE_00014", inst->getName(context), index, tables.size(), __LINE__);
  }
  return !value.isNull();
}
