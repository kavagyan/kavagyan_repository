...
bool Table::add(
    const myNode *node, const myNode *nodePointer, const Value &value,
    unsigned int source,
  ...
  if (index < tables.size()) {
    if (tables[index]) {
      return tables[index]->set(nodePointer, node, value, value, source,
                                sadPrecedence, level);
    }
  }
  ...
  if (instance().getAttr("err")) {
    ...
    std::cout("NOTE_00014", inst->getName(context), index, tables.size(),
              __LINE__);
  }
  return !value.isNull();
}
